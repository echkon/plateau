...
def plot_binned_average(ax, x_data, y_data, bin_size=2, poly_degree=6, label=None, marker_color='yellow', line_color='red', fix_first_point=True):
    """
    Calculates and plots a smooth trendline for data on a given axis.
    """
    x = np.array(x_data)
    y = np.array(y_data)
    ...
    # --- Plotting ---
    # MODIFIED: Changed alpha from 0.5 to 1.0 for solid markers
...
def create_plots():
    ...
    if pauli_layers:
        ...
        plot_binned_average(
            ax, pauli_layers[15:], pauli_variances[15:],
    ...
    if hea_layers:
        fig, ax = plt.subplots(figsize=(10, 8))
        plot_binned_average(
            ax, hea_layers[10:], hea_variances[10:],
